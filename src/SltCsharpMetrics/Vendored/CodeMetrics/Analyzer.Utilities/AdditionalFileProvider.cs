...
internal sealed class AdditionalFileProvider
{
	...
	{
		return new AdditionalFileProvider(options.AdditionalFiles);
	}
	...
	public AdditionalText? GetFile(string fileName)
	{
		...
		return _additionalFiles.FirstOrDefault((AdditionalText x) => Path.GetFileName(x.Path).Equals(fileName2, StringComparison.OrdinalIgnoreCase));
	}
	...
	public IEnumerable<AdditionalText> GetMatchingFiles(string pattern)
	{
		...
		return _additionalFiles.Where((AdditionalText x) => Regex.IsMatch(Path.GetFileName(x.Path), pattern2, RegexOptions.IgnoreCase));
	}
}
