...
internal interface ICategorizedAnalyzerConfigOptions
{
	...
	T GetOptionValue<T>(string optionName, SyntaxTree? tree, DiagnosticDescriptor? rule, CategorizedAnalyzerConfigOptionsExtensions.TryParseValue<T> tryParseValue, T defaultValue, OptionKind kind = OptionKind.DotnetCodeQuality);

	T GetOptionValue<T, TArg>(string optionName, SyntaxTree? tree, DiagnosticDescriptor? rule, CategorizedAnalyzerConfigOptionsExtensions.TryParseValue<T, TArg> tryParseValue, TArg arg, T defaultValue, OptionKind kind = OptionKind.DotnetCodeQuality);
}
